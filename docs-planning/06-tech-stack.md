...
| 层级 | 技术 | 版本 | 选型理由 |
|------|------|------|---------|
| 表现层 | Vue 3 + Vite + Tailwind CSS | Vue 3.4 | 响应式系统适合对话流式更新，Vite HMR 开发效率高 |
...
**Vue 3** 的响应式系统很适合"对话流式更新 + 状态面板实时刷新"的场景。`ref()` 和 `computed()` 能很自然地绑定 WebSocket 推过来的审计日志数据，不用手动操作 DOM。

**Vite** 作为构建工具，开发服务器启动快、HMR 即时刷新，对"边调后端边改前端"的协作模式很友好。
...
如果团队更熟悉 React，**React 18 + Next.js + Tailwind** 也完全可行。只是 Next.js 对纯客户端应用来说有点重，Vite 更轻量。

如果追求极致简单，**原生 HTML + Alpine.js** 也能跑，但图中组件交互复杂度（实时日志流、会话 CRUD、Markdown 渲染）已经超出 Alpine 的舒适区，不建议。

### 一句话建议

Vue 3 + Vite + Tailwind，配合 `axios`（HTTP）和 `socket.io-client`（WebSocket），一周能搭出全部界面。
